...
public class ApkV1Builder extends BaseBuilder {
    public ApkV1Builder(Context context, String packageName) {
        ...
        if (reader == null) CrashLog.logError(errorMessage, null);
        else {
            name = reader.device;
    ...
    @Override public Template build() {
        if (isSuccessBuild) return Template.build(this);
        else return null;
    }
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                ...
                switch (eventType) {
                    ...
                    case XmlPullParser.END_TAG:
                        ...
                        if (xppName.equalsIgnoreCase("device")) this.device = value;
                        else if (xppName.equalsIgnoreCase("author")) this.author = value;
                        else if (xppName.equalsIgnoreCase("topx")) this.tx = parseInt(value);
                        else if (xppName.equalsIgnoreCase("topy")) this.ty = parseInt(value);
                        else if (xppName.equalsIgnoreCase("botx")) this.bx = parseInt(value);
