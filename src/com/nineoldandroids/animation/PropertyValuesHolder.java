...
public class PropertyValuesHolder implements Cloneable {
	 ...
	 * 
	 * @param propertyName
	 *            The name of the property associated with this set of values.
	 *            This can be the actual property name to be used when using a
	 *            ObjectAnimator object, or just a name used to get animated
	 *            values, such as if this object is used with an ValueAnimator
	 *            object.
	 * @param values
	 ...
	 * 
	 * @param property
	 *            The property associated with this set of values. Should not be
	 *            null.
	 * @param values
	 *            The set of values to animate between.
	 */
	 ...
	 * @param prefix
	 *            "set" or "get", depending on whether we need a setter or
	 *            getter.
	 * @param valueType
	 *            The type of the parameter (in the case of a setter). This type
	 *            is derived from the values set on this PropertyValuesHolder.
	 *            This type is used as a first guess at the parameter type, but
	 *            we check for methods with several different types to avoid
	 *            problems with slight mis-matches between supplied values and
	 *            actual value types used on the setter.
	...
	private Method getPropertyFunction(Class targetClass, String prefix,
		...
		if (valueType == null) {
			try {
				...
				/*
				 * The native implementation uses JNI to do reflection, which
				 * allows access to private methods. getDeclaredMethod(..) does
				 * not find superclass methods, so it's implemented as a
				 * fallback.
				 */
				try {
			...
			for (Class typeVariant : typeVariants) {
				...
				try {
					...
					/*
					 * The native implementation uses JNI to do reflection,
					 * which allows access to private methods.
					 * getDeclaredMethod(..) does not find superclass methods,
					 * so it's implemented as a fallback.
					 */
					try {
	 ...
	 * @param valueType
	 *            The type of parameter passed into the method (null for
	 *            getter).
	 * @return Method the method associated with mPropertyName.
	 */
	 ...
	 *
	 * @param target
	 *            The target object from which the current value should be
	 *            extracted.
	 * @param kf
	 ...
	 *
	 * @param prefix
	 *            The precursor to the method name, before the property name
	 *            begins, typically "set" or "get".
	 * @param propertyName
	 *            The name of the property that represents the bulk of the
	 *            method name after the prefix. The first letter of this word
	 *            will be capitalized in the resulting method name.
